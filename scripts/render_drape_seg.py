...
ap.add_argument("--out", default="/tmp/duct_seg")
...
from duct_sim.build_b_physx import build_duct_b_physx_segmented  # noqa: E402
...
info = build_duct_b_physx_segmented(stage, "/World/DuctB", shifted)
...
for pr in info["patches"]:
    UsdGeom.Xformable(stage.GetPrimAtPath(pr)).AddTranslateOp().Set(
        Gf.Vec3d(0.0, 0.0, lift - shifted.drop_to_floor))

print(f"[physx] {spec.describe()}", flush=True)
print(f"[physx] {info['n_patches']} separate cloth patches, "
      f"{len(info['attachments'])} seams", flush=True)
...
print(f"[physx] frames={frames}  patches={info['n_patches']}", flush=True)
sys.stdout.flush()
app.close()
