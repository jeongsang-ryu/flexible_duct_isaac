...
print("\n stiff  force  angdamp |  spread   grabbed_dy  ends_dy  ratio | verdict", flush=True)
print("-" * 76, flush=True)

CASES = [  # (bend_stiffness, pull_force_N, angular_damping)
    (1.0, 5.0, 0.0),     # control: no damping -> should still roll away
    (1.0, 5.0, 2.0),
    (1.0, 5.0, 10.0),
    (1.0, 5.0, 40.0),
    (0.2, 5.0, 10.0),
...
for stiff, force, angdamp in CASES:
    ...
    print(f"{stiff:6.2f} {force:6.1f} {angdamp:8.1f} | {spread:7.4f}  {grabbed:9.4f}"
          f"  {ends:7.4f}  {ratio:5.2f} | {verdict}", flush=True)
    sim.stop()
