...
ap.add_argument("--out", default="/tmp/duct_drape_b")
...
# The fabric MUST be constructed after the hoops are moved into place. It
# seeds itself from the hoops' real poses, so building it first (while they are
# still at the authored floor height) and only then lifting them onto the bar
# teleports every sewn seam 2 m upward on the first step, stretching the sleeve
# ~10x and detonating it. The floor scene never showed this because nothing
# moves between construction and the first step.
fabric = NewtonFabric(spec, ring_world)
print(f"[drapeB] Newton cloth {fabric.n_particles} particles", flush=True)
from pxr import Vt  # noqa: E402
fmesh = UsdGeom.Mesh.Define(stage, "/World/DuctA/fabric_sim")
fmesh.CreateFaceVertexCountsAttr(Vt.IntArray([3] * len(fabric.faces)))
fmesh.CreateFaceVertexIndicesAttr(Vt.IntArray(fabric.faces.flatten().tolist()))
fmesh.CreateDoubleSidedAttr(True)
fpts = fmesh.CreatePointsAttr()
...
for i in range(args.steps):
    ...
    fabric.step(1.0 / 60.0)
    fpts.Set(Vt.Vec3fArray.FromNumpy(fabric.positions().astype(np.float32)))
    if i % args.every == 0:
...
print(f"[drapeB] stiffness={args.stiffness} frames={frames} -> {args.out}", flush=True)
print(f"[drapeB] top z={pos[:,2].max():.3f}  lowest z={pos[:,2].min():.3f}  "
